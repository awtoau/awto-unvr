...
Feature-parity target = stock 2015.07 commands **plus** everything we RE'd this
session that the stock U-Boot never exposed.
...
| **Bay power on/off** | PCA9575 **@0x21** pins 0-3 = bay1-4 **pwren** (gpio 480-483) | i2c + pca953x GPIO; a `bay` command (on/off/status per bay) | stock U-Boot has none — **new** |
...
| **Bay activity LEDs** | SGPO (74VHC595 `UB20`), sgpo lines 16-23 | stock U-Boot `sgpo` cmd equivalent | port from stock U-Boot |
...
| **Identity EEPROM** | AT24C64 @0x57 (2-byte addr) | i2c read + decode (sysid/MAC/serial) | partial in stock U-Boot |
| **SFP module** | Finisar FTLX8571D3BCL, EEPROM 0x50/0x51 (mux ch1) | i2c read + SFF-8079/8472 decode (DDM: temp, Vcc, TX/RX power) | **new** |
| **SFP retimer** | `br410` (per stock `sfpd`), raw i2c writes | optional | **new** |
...
| **NAND** | MT29F8G08, al-nand @0xfa100000 | `nand` — needs the AL NAND driver ported | port from stock U-Boot |
| **SATA** | 2× AHCI (`1c36:0031`) behind internal PCIe | PCIe enum + AHCI + `scsi`/`sata` (**the stock U-Boot can't read SATA — we fix this**) | port PCIe glue |
| **Ethernet 1G** | al_eth + AR8033 PHY (MDIO addr 4) | al_eth port + `mdio`/`mii`, ping/tftp | port from stock U-Boot |
| **Ethernet 10G** | SFP+ via SerDes, no PHY | serdes cfg + link mgmt | port from stock U-Boot |
...
| **Pinmux** | MUIO mux (73 ifaces) | `muio_mux` equivalent | port from stock U-Boot |
| **Boot instance / TOC** | flash TOC A/B/C/D | `flash_contents_*` equivalent | port from stock U-Boot |

...
- **HAL (the engine):** `delroth-alpine_hal/` — github.com/delroth/alpine_hal @ eb6b9f1. The full
  **platform-neutral** Annapurna HAL: `include/`+`drivers/` for eth, udma, udma_fast, serdes,
  iofic, pcie, **ddr**, common, + a **`plat_api/`** abstraction and `samples/eth.c` (bring-up
  reference), `samples/dummy_plat.c` (platform-port template). Tri-licensed (Annapurna/GPL-2/BSD).

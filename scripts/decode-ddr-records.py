...
def hexs(off, n): return " ".join("%02x" % b for b in data[off:off+n])
...
out("  +1 spd_i2c_addr = 0x%02x  %s" % (spd_addr, "(0xff => use bootstrap i2c_preload_addr, live=0x57)" if spd_addr==0xff else ""))
out("  +2 spd_off      = 0x%04x  (u16 LE)  -> SPD image read here on the SAME device" % spd_off)
out("  +4 aux_i2c_addr = 0x%02x  %s" % (aux_addr, "(0x00 => no 2nd-rank/dimm probe => dimms=1)" if aux_addr==0 else ""))
...
out("  +1 gpio_pin = 0x%02x  %s" % (bb[1], "(0xff => DISABLED: no GPIO drives DRAM voltage; DDR4 fixed 1.2 V)" if bb[1]==0xff else "(active pin < 0x38)"))
out("  +2 polarity = 0x%02x  %s" % (bb[2], "(unused, pin disabled)" if bb[1]==0xff else ""))

# ---- 0xCC impedance override : FUN_f2200a58 checks buf[+0x0e]==0xCC
cc = data[BASE+0x0e:BASE+0x0e+0x16]
out("\n[0xCC] impedance override @0x%03x (0x16 B): %s" % (BASE+0x0e, hexs(BASE+0x0e, 0x16)))
assert cc[0] == 0xCC
dqs_sel = cc[1]
tbl = cc[2:2+20]  # 20-byte table @ base+0x10
out("  +0 magic       = 0x%02x" % cc[0])
out("  +1 dqs_sel     = 0x%02x  %s" % (dqs_sel,
    "(==1 => dqs_res=PULL_DOWN_500OHM, dqsn_res=PULL_UP_500OHM)" if dqs_sel==1
...
sel = tbl[0:5]
out("\n  active 5-byte set (dimms=1,ranks/dimm=1 -> idx 0): %s" % " ".join("%02x" % b for b in sel))
odt_v, odt_dyn_v, dic_v, phy_odt_v, phy_rout_v = sel
out("    [0] odt      EEPROM=0x%02x  RZQ/%d = %d ohm  -> enum %s" % (odt_v, odt_v, 240//odt_v if odt_v else 0, lookup(t_odt, odt_v)))
out("    [1] odt_dyn  EEPROM=0x%02x  %s -> enum %s" % (odt_dyn_v, ("RZQ/%d = %d ohm"%(odt_dyn_v,240//odt_dyn_v)) if odt_dyn_v else "divider 0 = DISABLED", lookup(t_odt_dyn, odt_dyn_v)))
out("    [2] dic      EEPROM=0x%02x  RZQ/%d = %d ohm  -> enum %s" % (dic_v, dic_v, 240//dic_v if dic_v else 0, lookup(t_dic, dic_v)))
out("    [3] phy_odt  EEPROM=0x%02x  = %d ohm         -> enum %s" % (phy_odt_v, phy_odt_v, lookup(t_phy_odt, phy_odt_v)))
out("    [4] phy_rout EEPROM=0x%02x  = %d ohm         -> enum %s" % (phy_rout_v, phy_rout_v, lookup(t_phy_rout, phy_rout_v)))
...
out("\nCRC gate:")
out("  block1 CRC over 0..0x%02x: calc=0x%04x stored=0x%04x  %s" % (crc_len1-1, c1, c1_stored, "OK" if c1==c1_stored else "MISMATCH"))
out("  block2 CRC over 0x80..0xfd: calc=0x%04x stored=0x%04x  %s" % (c2, c2_stored, "OK" if c2==c2_stored else "MISMATCH"))
...
bg_bits    = b4 >> 6
ecc  = (b13 & 0x18) != 0
mtype = {1:"RDIMM",2:"UDIMM",3:"SODIMM",4:"LRDIMM"}.get(b3 & 0xf, "0x%x"%(b3&0xf))
...
out("  byte 0x0d = 0x%02x  data_width    = %s (raw&7=%d) ; ecc = %s" % (b13, data_width, dw_code, ecc))
out("  byte 0x04 = 0x%02x  bank_bits     = %d (%d banks) ; bank_group_bits = %d (%d groups)"
    % (b4, banks_bits, 1<<banks_bits, bg_bits, 1<<bg_bits))
out("  byte 0x05 = 0x%02x  row_bits      = %d ; col_bits = %d" % (b5, rows, cols))
dens_bits = rows + cols + banks_bits + bg_bits + {"x4":2,"x8":3,"x16":4,"x32":5}[dev_w]
out("  derived per-device density = 2^%d bits = %d Gbit  (JEDEC byte4[3:0] density code = %d, IGNORED by parser)"
...
def sc(i):  # signed char
    v = B(i); return v-256 if v>=128 else v
out("\nTiming (MTB=%d ps, FTB=1 ps ; byte0x11 timebase=0x%02x):" % (MTB, tb))
tck_mtb = B(0x12); tck_ftb = sc(0x7d)
...
import math
def clk(ps): return math.ceil(ps/tck) if tck else 0
out("  byte 0x12/0x7d  tCKAVGmin = %d ps  => %d MT/s (fastest the SPD allows)" % (tck, round(2e6/tck)))
out("  byte 0x14..17   CAS map = 0x%08x  supported CL = %s" % (casmap, cls))
out("  byte 0x18/0x7b  tAAmin  = %5d ps  = CL %d (ceil tAA/tCK; next supported)" % (taa, clk(taa)))
...
out("  byte 0x1e/0x1f  tRFC1   = %6d ps = %d ns  (8 Gbit JEDEC=350 ns)" % (trfc1, trfc1//1000))
out("  byte 0x20/0x21  tRFC2   = %6d ps = %d ns  (8 Gbit JEDEC=260 ns)" % (trfc2, trfc2//1000))
out("  byte 0x22/0x23  tRFC4   = %6d ps = %d ns  (8 Gbit JEDEC=160 ns)" % (trfc4, trfc4//1000))
...
out("Raw SPD FTB bytes 0x75..0x7d: " + " ".join("%02x"%B(i) for i in range(0x75,0x7e)))
...
u6 = cols
while u5 < u6:
    am[u5-3] = iv & 0xff; iv += 1; u5 += 1
while u5 < 0xd:
    am[u5-3] = 0xff; u5 += 1
cVar1 = iv & 0xff
# cs (rank) map : ranks==1 here -> NC,NC
if ranks == 4:
    am[0x21] = cVar1; iv += 2; am[0x22] = (cVar1+1)&0xff
elif ranks == 2:
    am[0x21], am[0x22] = (0xff, cVar1) if False else (cVar1, 0xff)
    iv += 1
else:
    am[0x21] = 0xff; am[0x22] = 0xff
# row map fill part 1 (while iv<=0x10)
iv8 = iv; p = 0xf
while True:
    d = iv8 - iv; cVar1 = iv8 & 0xff
    if iv8 > 0x10: break
    am[0xf + d] = cVar1; iv8 += 1
# bank map
bk = banks_bits
for j in range(bk):
...
while u5b < rows:
    am[0xf + u5b] = (base + u5b) & 0xff; u5b += 1
while u5b < 0x12:
    am[0xf + u5b] = 0xff; u5b += 1
am[0x24] = 0  # *(u32)(am+0x24)=0

def amslice(lo, hi): return " ".join("%02x"%am[i] for i in range(lo,hi+1))
...
out("  (0xff = AL_DDR_ADDRMAP_NC ; row bits 0-2 -> sys 14-16, bank -> 17-18, row 3-15 -> 19-31)")

# ------------------------------------------------------------------ impedance_ctrl (final)
out("\n" + "=" * 72)
out("impedance_ctrl  (FUN_f2200a58 -> struct al_ddr_init_cfg_impedance_ctrl, cfg+0xb8)")
out("=" * 72)
ODT_E  = {0:"DIS",1:"RZQ2",2:"RZQ4",3:"RZQ6",4:"RZQ8",5:"RZQ12",6:"RZQ1",7:"RZQ5",8:"RZQ3",9:"RZQ7"}
DYN_E  = {0:"DIS",1:"RZQ2",2:"RZQ4",3:"RZQ1",4:"HI_Z"}
DIC_E  = {0:"RZQ6",1:"RZQ7",2:"RZQ5"}
odt_e  = lookup(t_odt, odt_v); dyn_e = lookup(t_odt_dyn, odt_dyn_v); dic_e = lookup(t_dic, dic_v)
podt_e = lookup(t_phy_odt, phy_odt_v); prout_e = lookup(t_phy_rout, phy_rout_v)
out("  dic               = %d  AL_DDR_DIC_%s  (RZQ/%d = %d ohm)" % (dic_e, DIC_E[dic_e], dic_v, 240//dic_v))
out("  odt (RTT_NOM)     = %d  AL_DDR_ODT_%s  (RZQ/%d = %d ohm)" % (odt_e, ODT_E[odt_e], odt_v, 240//odt_v))
...
out("  ddr_size = ranks(%d) << (row+col+bank+bg = %d) * bus_bytes(%d) = %d bytes = %d GiB"
...
def freq_enum(tck_ps):
    if tck_ps == 0x753: return 1          # 1875 -> 1066 (also the no-match default=1)
    if tck_ps == 0x5dc: return 2          # 1500 -> 1333
    if tck_ps == 0x4e2: return 3          # 1250 -> 1600
    if tck_ps in (0x442, 0x42f): return 4 # 1090/1071 -> 1866
    if 0x3a9 <= tck_ps < 0x3a9+0x85: return 5   # 2133
    if 0x341 <= tck_ps < 0x341+0x68: return 6   # 2400
    return 1
fe = freq_enum(tck)
out("  SPD tCKAVGmin = %d ps (0x%x) -> speed-bin enum = %d = AL_DDR_FREQ_%s (fastest the DIMM allows)"
    % (tck, tck, fe, FREQ_E.get(fe,"?")))
out("  CL  (tmg+0x38) = %d  ;  CWL (tmg+0x3c, FUN_f2200e74 @tCK=%d) = %d" % (clk(taa), tck, 10 if 0x42e<=tck<0x4e2 else 0))
out("  NOTE: running freq = bootstrap ddr_pll_freq (live, still open); SPD CAPS it at 1866.")

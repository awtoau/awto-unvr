...
def parse_toc(b: bytes, toc: int) -> list[dict]:
    ...
    for n in range(count):
        ...
        if ooff + 0x48 <= len(b) and struct.unpack_from("<I", b, ooff)[0] == IMG_MAGIC:
            ...
            ppayload = ooff + 0x48
        out.append(dict(name=name, off=ooff, size=osize,
                        psize=psize, ppayload=ppayload))
    return out
...
def diff_region(a: bytes, b: bytes, ra: tuple[int, int], rb: tuple[int, int]) -> str:
    ...
    return (f"CHANGED    old={la} new={lb} (delta {lb - la:+d}); "
...
def main() -> int:
    ...
    ap.add_argument("--carve", type=Path, default=None,
    ...
    for name in order:
        ...
        if o["ppayload"] is None or n["ppayload"] is None:
            ...
            if ra[1] <= ra[0] or rb[1] <= rb[0]:
                log("%-10s TOC off 0x%06x size 0x%06x (outside carved file)",
                    name, o["off"], o["size"])
                continue

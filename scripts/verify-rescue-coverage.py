...
def ext4_allocated(img: Path) -> tuple[list[tuple[int, int]], int, int]:
    ...
    text = p.stdout
    bs = int(re.search(r"^Block size:\s+(\d+)", text, re.MULTILINE).group(1))
    bc = int(re.search(r"^Block count:\s+(\d+)", text, re.MULTILINE).group(1))

    free: list[tuple[int, int]] = []
    for m in re.finditer(r"^\s*Free blocks: (.*)$", text, re.MULTILINE):
...
def write_domain(alloc: list[tuple[int, int]], size: int, out: Path) -> None:
    ...
    """
    lines = [
        "# Domain mapfile: ext4-allocated ranges only.",
...
if __name__ == "__main__":
    ...
    log(
        f"INCOMPLETE - {human(total_missing)} of allocated data not yet rescued, "
        f"in {len(missing)} range(s):",
        "ERROR",
    )

...
import hashlib, http.server, socketserver, sys, threading
...
def main():
    ...
    if rc != 0:
        ...
        httpd.shutdown(); s.close(); return 1
    ...
    for src, dst in FILES:
        ...
        rc, out = con.sh(s, f"curl -fsS {url} -o {b}.new && echo FETCH_OK || echo FETCH_FAIL", 120)
        if "FETCH_OK" not in out:
            log(f"FETCH FAILED for {src}: {out}"); ok = False; break
        rc, h = con.sh(s, f"sha256sum {b}.new | cut -d' ' -f1", 30)
        got = h.split()[0] if h.split() else ""
        if got != local_hash[dst]:
            log(f"HASH MISMATCH {dst}: box {got} != local {local_hash[dst]} — NOT installing"); ok = False; break
        log(f"{dst}: sha256 OK — installing")
        con.sh(s, f"mv {b}.new {b}; sync; echo installed")

    httpd.shutdown()
    if ok:
        _, ls = con.sh(s, "ls -la --time-style=+%H:%M /boot/uImage-unvr-ea16-7.1-fedora-gz* "

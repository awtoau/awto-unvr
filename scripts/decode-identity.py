...
"""
import os, sys, struct, uuid, zlib, math, collections, itertools, json, logging

REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DUMP = os.path.join(REPO, "images/mtd/"
    "UNVR-74acb941a811-sysidea16-20260815-212945-post-5.1.25-final")
LOGDIR = os.path.join(REPO, "tmp/logs")
os.makedirs(LOGDIR, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(message)s",
    handlers=[logging.FileHandler(os.path.join(LOGDIR, "decode-identity.log")),
...
def blank_verdict(name):
    d = part(name); c = collections.Counter(d)
    ff = c.get(0xff, 0); z = c.get(0, 0)
    if ff == len(d): v = "100% 0xFF (erased/blank)"
    elif z == len(d): v = "100% 0x00 (actively zeroed)"
    else: v = f"WRITTEN ff={ff*100/len(d):.1f}% zero={z*100/len(d):.1f}% distinct={len(c)}"
...
def decode_eeprom():
    ...
    boardid = d[0x0c:0x0e].hex(); hwrev = d[0x0e:0x10].hex(); devid = d[0x10:0x14].hex()
    ...
    fm["fields"] += [
        ...
        {"off":"0x0006","len":6,"name":"MAC+2 (base MAC, first octet +2)","value":mac2},
    ...
    r_ver = d[0x800c:0x800e].hex(); r_ver2 = d[0x800e:0x8010].hex()
    r_hwrev = d[0x8010:0x8012].hex(); r_boardid = d[0x8012:0x8014].hex()
    ...
    for lo, hi, desc in [(0x8008, 0x8008+blen, "0x8008..+len"),
    ...
    fm["fields"] += [
        ...
        {"off":"0x800C","len":4,"name":"version words","value":f"0x{r_ver} 0x{r_ver2}"},
    ...
    tver = d[0xd008]; tlen = int.from_bytes(d[0xd009:0xd00b], "big")
    log(f"0xd000 magic 'TlvInfo\\0' ver={tver} total_len=0x{tlen:x}")
    o = 0xd00b; endt = 0xd00b + tlen
    tlvs = []
    while o < endt:
        t = d[o]; l = d[o+1]; v = d[o+2:o+2+l]
    ...
    for lo, hi, desc in [(0xd000, endt-4, "0xd000..crc"),
    ...
    fm["tlvinfo"] = {"off":"0xd000","version":tver,"total_len":tlen,
                     "crc":f"0x{crc_stored_t:08x}","tlvs":tlvs}
    ...
    fm["fields"].append({"off":"0x9000","len":len(blob),
        "name":"unidentified high-entropy blob","value":f"entropy {entropy(blob):.2f} b/byte"})
    # RSA private key @0xe004 - report ONLY structure, NEVER bytes
    log("\n-- RSA private key @0xe004 (SSH-wire format) [NO KEY MATERIAL] --")
    hdr = d[0xe004:0xe00b]
    log(f"0xe004 7-byte prefix   {hdr.hex()} ('{''.join(chr(b) if 32<=b<127 else '.' for b in hdr)}')")
    o = 0xe00b; end = 0xe330
    while d[end-1] == 0xff: end -= 1
    fields = []
    while o < end and o+4 <= end:
        ln = int.from_bytes(d[o:o+4], "big")
        if ln == 0 or ln > 4096 or o+4+ln > end: break
        fields.append((o, ln, d[o+4:o+4+ln])); o += 4 + ln
    nums = []
    ...
    verified = any(a*b == N for a, b in itertools.combinations(primes, 2)) if N else False
...
def decode_config():
    ...
    logbs = struct.unpack_from("<I", sb, 0x18)[0]; bs = 1024 << logbs
    ...
    return {"partition":"mtd06 config","fs":"ext4","uuid":str(u),
            "label":label,"last_mounted":lastmnt,"block_size":bs,"blocks":blkcnt}

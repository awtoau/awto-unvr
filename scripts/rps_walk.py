...
def rod_end_v(): return ROD_V + ROD_L
def in_rodata(v): return ROD_V <= v < ROD_V + ROD_L
def in_dro(v): return DRO_V <= v < DRO_V + DRO_L
def in_data(v): return DAT_V <= v < DAT_V + 0x4000

def v2f(v):
    if in_rodata(v): return v  # vaddr==fileoff
    if in_dro(v): return v - (DRO_V - DRO_F)
    if in_data(v): return v - (DAT_V - DAT_F)
    return None

def cstr(fileoff, maxlen=64):
    if fileoff is None or fileoff < 0 or fileoff >= len(data): return None
    end = data.find(b"\x00", fileoff)
    if end < 0: end = fileoff + maxlen
    s = data[fileoff:end]
...
def main():
    ...
    if cmd == "scan":
        ...
        for i in range(n):
            ...
            if in_rodata(v):
                print("DRO file 0x%05x vaddr 0x%05x : %s" % (fo, DRO_V + i*8, annotate(v)))
    elif cmd == "findref":
        ...
        while True:
            ...
            if idx < 0: break
            sect = "rodata" if ROD_F <= idx < ROD_F+ROD_L else ("DRO" if DRO_F <= idx < DRO_F+DRO_L else ("data" if DAT_F <= idx else "?"))
        ...
        for i in range(cnt):
            ...
            print("  [%2d] file 0x%05x vaddr 0x%05x = %s" % (i, fo+i*8, (fo+i*8)+ (DRO_V-DRO_F if DRO_F<=fo<DRO_F+DRO_L else 0), annotate(v)))
    elif cmd == "dump":
        fo = int(sys.argv[2], 0); cnt = int(sys.argv[3], 0) if len(sys.argv)>3 else 128
        import binascii

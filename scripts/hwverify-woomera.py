...
CHECKS = [
    ...
    (
        "drivers",
        "lsmod | grep -iE 'at24|ina2xx|pmbus|adt7475|pca953x|s35390|i2c_desig|sp805|nvmem' || echo '(none of the target modules loaded)'",
    ),
    ("i2c buses", "i2cdetect -l 2>/dev/null || echo 'i2c-tools missing'"),
    (
        "i2c scan",
        "for n in $(i2cdetect -l 2>/dev/null | sed -n 's/^i2c-\\([0-9]*\\).*/\\1/p'); do echo \"== i2c-$n ==\"; i2cdetect -y -r $n 2>/dev/null; done",
    ),
    (
        "hwmon",
        'for h in /sys/class/hwmon/hwmon*; do echo -n "$h: "; cat $h/name 2>/dev/null; done',
    ),
    (
        "rtc",
        "ls /sys/class/rtc 2>/dev/null; hwclock -r 2>/dev/null || echo '(no rtc read)'",
    ),
    ("leds", "ls /sys/class/leds 2>/dev/null"),
    (
        "watchdog",
        "ls /sys/class/watchdog 2>/dev/null; cat /sys/class/watchdog/watchdog0/identity 2>/dev/null",
    ),
    (
        "nvmem",
        "ls -l /sys/bus/nvmem/devices 2>/dev/null; ls /sys/bus/i2c/devices/*/eeprom 2>/dev/null || echo '(no at24 eeprom sysfs — DTB lacks eeprom@57 or at24 not loaded)'",
    ),
    (
        "gpio",
        "ls /sys/bus/i2c/drivers/pca953x 2>/dev/null; gpioinfo 2>/dev/null | grep -iE 'chip|pca' | head",
    ),

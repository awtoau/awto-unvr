...
def curl(url, dest):
    return subprocess.run(["curl", "-fL", "-C", "-", "-o", str(dest), url]).returncode == 0
...
def main():
    ...
    for d, fn, md5 in FILES:
        ...
        if not curl(f"{BASE}/{d}/{fn}", tar):
            log(f"  DOWNLOAD FAILED"); bad.append(fn); continue
        # verify: prefer sha256 sidecar, else md5 fallback
        side = tar.with_suffix(tar.suffix + ".sha256")
        verified = False
        if curl(f"{BASE}/{d}/{fn}.sha256", side) and side.exists() and side.stat().st_size:
        ...
        try:
            ...
            log(f"  UNPACK FAILED: {e}"); bad.append(fn + "(unpack)")

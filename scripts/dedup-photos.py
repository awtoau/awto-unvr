...
def main():
    ...
    ap.add_argument("--thresh", type=int, default=5, help="max Hamming dist for near-dup")
    a = ap.parse_args()

    jpgs = sorted(DIR.glob("*.jpg"))
    info = {p: (p.stat().st_size, hashlib.sha256(p.read_bytes()).hexdigest(), dhash(p))
            for p in jpgs}
    ...
    def find(x):
        ...
        return x
    def union(a_, b_): parent[find(a_)] = find(b_)

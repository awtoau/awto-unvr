...
static int al_mdio_read(struct mii_bus *bp, int mii_id, int reg)
{
	...
	while (timeout > 0) {
		rc = al_eth_mdio_read(&adapter->hal_adapter, mii_id,
			0, reg, &value);
		if (rc == 0)
			return value;
		timeout -= 10;
...
static int al_mdio_write(struct mii_bus *bp, int mii_id, int reg, u16 val)
{
	...
	while (timeout > 0) {
		rc = al_eth_mdio_write(&adapter->hal_adapter, mii_id,
			0, reg, val);
		if (rc == 0)
			return 0;
		timeout -= 10;
	...
	if (rc)
		netdev_err(adapter->netdev, "MDIO C22 write phy %d reg %d failed\n",
			   mii_id, reg);
...
al_eth_rx_poll(struct napi_struct *napi, int budget)
{
	...
	do {
		...
		if (likely(adapter->netdev->features & NETIF_F_RXHASH)) {
			...
			/*
			 * The HW performs a 16-bit hash. Expand to 32-bit
			 * for the kernel's skb hash.
			 */
			 ...
			 struct ethtool_pauseparam *pause)
{
	...
	if (pause->autoneg) {
		...
		linkmode_zero(new_adv);

		/* capture old pause advertising */
		if (linkmode_test_bit(ETHTOOL_LINK_MODE_Pause_BIT, phydev->advertising))
			linkmode_set_bit(ETHTOOL_LINK_MODE_Pause_BIT, old_adv);
		if (linkmode_test_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, phydev->advertising))
			linkmode_set_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, old_adv);

		/* set new pause advertising */
		if (newadv & ADVERTISED_Pause)
			linkmode_set_bit(ETHTOOL_LINK_MODE_Pause_BIT, new_adv);
		if (newadv & ADVERTISED_Asym_Pause)
			linkmode_set_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, new_adv);

		if (!linkmode_equal(old_adv, new_adv)) {

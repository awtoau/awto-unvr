...
extern void al_eth_initial_moderation_table_restore_default(void);

static ssize_t al_eth_store_intr_moderation_restore_default(struct device *dev,
					      struct device_attribute *attr,
					      const char *buf, size_t len)
{
...
static struct device_attribute dev_attr_intr_moderation_restore_default = {
	...
	.show = NULL,
	.store = al_eth_store_intr_moderation_restore_default,
};

static ssize_t al_eth_show_intr_moderation(
	struct device *dev,
	struct device_attribute *attr,
	char *buf);

static ssize_t al_eth_store_intr_moderation(
	struct device *dev,
	struct device_attribute *attr,
	const char *buf,
	size_t count);
...
#define INTR_MODERATION_PREPARE_ATTR(_name, _type) {				    \
	__ATTR(intr_moderation_##_name, (S_IRUGO | S_IWUSR | S_IWGRP),			    \
		al_eth_show_intr_moderation, al_eth_store_intr_moderation), \
		(void*)_type }
...
/******************************************************************************
 *****************************************************************************/
extern void al_eth_get_intr_moderation_entry(enum al_eth_intr_moderation_level level,
		struct al_eth_intr_moderation_entry *entry);

extern void al_eth_init_intr_moderation_entry(enum al_eth_intr_moderation_level level,
		struct al_eth_intr_moderation_entry *entry);
	...
	char *buf)
{
	struct al_eth_intr_moderation_entry entry;
	struct dev_ext_attribute *ea = to_ext_attr(attr);
	enum al_eth_intr_moderation_level level =
			(enum al_eth_intr_moderation_level)ea->var;
	ssize_t rc = 0;

	al_eth_get_intr_moderation_entry(level, &entry);
	...
	size_t count)
{
	...
	al_eth_init_intr_moderation_entry(level, &entry);

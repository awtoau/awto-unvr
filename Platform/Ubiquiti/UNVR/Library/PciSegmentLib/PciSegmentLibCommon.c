/** @file
  ...
  UNVR fork (Platform/Ubiquiti/UNVR/Library/PciSegmentLib/, docs/uefi.md
  P1.5): copied from MdePkg/Library/PciSegmentLibSegmentInfo verbatim,
  one change in PciSegmentLibGetEcamAddress() below - external PCIe0
  (segment 1) aliases the same real device (the ASM1042A xHCI) across
  every devfn (confirmed live 2026-09-03, PciBusDxe discovered
  1B21:1142 at devfn 0 through 9 identically, crashed XhciDxe with a
  hard SError on a bogus alias's bad BAR). Every mainline
  pcie-designware-host.c-based Linux driver has a built-in guard for
  exactly this (dw_pcie_rd_other_conf(): devfn != 0 on the root
  complex's own bus is answered "not present" in software, never
  touched on real hardware) - this is EDK2's equivalent for the one
  segment that actually needs it.

  Copyright (c) 2017, Intel Corporation. All rights reserved.<BR>
  Portions Copyright (c) 2026, Awto / Daniel Tyrrell. All rights reserved.<BR>
...
//
// External PCIe0 is segment 1 (Platform/Ubiquiti/UNVR/Library/
// PciSegmentInfoLib) - only devfn 0/0 is real. All-0xFF matches the
// standard PCI "no device" vendor-ID convention; writes land here and
// are silently discarded, exactly as real unpopulated config space
// would behave.
//
#define UNVR_PCIE_EXT0_SEGMENT  1
STATIC UINT8  mUnvrDummyConfigSpace[4096];
STATIC BOOLEAN  mUnvrDummyConfigSpaceInit = FALSE;
...
{
  //
  // UNVR guard (see file header): external PCIe0 only has one real
  // device, at devfn 0/0. Any other devfn on that segment's bus is a
  // hardware-level alias of the same device, not a genuinely separate
  // one - redirect to a dummy, all-0xFF buffer instead of ever issuing
  // the real MMIO access, so it reads back as the standard PCI "no
  // device" response and writes land somewhere harmless.
  //
  if ((((PCI_SEGMENT_LIB_ADDRESS_STRUCTURE *)&Address)->Segment == UNVR_PCIE_EXT0_SEGMENT) &&
      ((((PCI_SEGMENT_LIB_ADDRESS_STRUCTURE *)&Address)->Device != 0) ||
       (((PCI_SEGMENT_LIB_ADDRESS_STRUCTURE *)&Address)->Function != 0)))
  {
    if (!mUnvrDummyConfigSpaceInit) {
      SetMem (mUnvrDummyConfigSpace, sizeof (mUnvrDummyConfigSpace), 0xFF);
      mUnvrDummyConfigSpaceInit = TRUE;
    }

    return PciSegmentLibVirtualAddress (
             (UINTN)&mUnvrDummyConfigSpace[((PCI_SEGMENT_LIB_ADDRESS_STRUCTURE *)&Address)->Register]
             );
  }

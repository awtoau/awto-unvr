 ...
 * @dw_of_cfg: Backing store for explicit raw SCL/SDA counts read from DT
 *	(i2c-{ss,fs,hs}-scl-{h,l}cnt-raw); scl_sda_cfg points here when set.
 *	awto: pins stock's proven s35390a-safe pld-bus timing, see
...
struct dw_i2c {
	...
	struct dw_scl_sda_cfg dw_of_cfg;
};

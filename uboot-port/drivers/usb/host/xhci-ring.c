...
union xhci_trb *xhci_wait_for_event(struct xhci_ctrl *ctrl, trb_type expected)
{
	...
	/* #140 DIAGNOSTIC (temporary - revert once root-caused): periodic
	 * visibility into the poll loop, scoped to TRB_COMPLETION waits only
	 * (fast paths like TRB_TRANSFER/TRB_NONE finish before the 200ms
	 * gate ever fires, so this stays silent for everything that already
	 * works - only the actual stall produces output). */
	unsigned long last_diag_ts = ts;
	...
	do {
		...
		if (expected == TRB_COMPLETION && get_timer(last_diag_ts) >= 200) {
			...
			printf("#140-diag: still waiting (%lu ms) event_ring deq=%p raw=%08x %08x %08x %08x cycle_state=%d db[0]=0x%08x\n",
			       get_timer(ts), event, le32_to_cpu(event->generic.field[0]),
			       le32_to_cpu(event->generic.field[1]), le32_to_cpu(event->generic.field[2]),
			       le32_to_cpu(event->generic.field[3]), ctrl->event_ring->cycle_state,

...
static int _xhci_alloc_device(struct usb_device *udev)
{
	 ...
	 * originates (xhci_wait_for_event -> -ETIMEDOUT). See the issue for
	 * why - the driver already cache-invalidates before reading the
	 * event ring, so this checks whether the doorbell ring is even
	 * reaching the controller, not a cache-visibility gap.
	 */
